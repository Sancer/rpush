...
describe Rpush::Daemon::Fcm::Delivery do
  let(:app) { Rpush::Fcm::App.create!(name: 'MyApp', auth_key: 'abc123') }
  let(:notification) { Rpush::Fcm::Notification.create!(app: app, registration_ids: ['xyz'], deliver_after: Time.now) }
  ...
  shared_examples_for 'a notification with some delivery failures' do
    let(:new_notification) { Rpush::Fcm::Notification.where('id != ?', notification.id).first }
    ...
    it 'creates a new notification for the unavailable devices' do
      ...
      expect(store).to receive(:create_fcm_notification).with(attrs, notification.data,
  ...
  describe 'a 200 response' do
    ...
    it 'reflects on any IDs which successfully received the notification' do
      ...
      expect(delivery).to receive(:reflect).with(:fcm_delivered_to_recipient, notification, '1')
      expect(delivery).not_to receive(:reflect).with(:fcm_delivered_to_recipient, notification, '2')
      perform_with_rescue
    end
    ...
    it 'reflects on any IDs which failed to receive the notification' do
      ...
      allow(response).to receive_messages(body: JSON.dump(body))
      allow(notification).to receive_messages(registration_ids: %w(1 2))
      expect(delivery).to receive(:reflect).with(:fcm_failed_to_recipient, notification, 'Err', '1')
      expect(delivery).not_to receive(:reflect).with(:fcm_failed_to_recipient, notification, anything, '2')
      perform_with_rescue
    end
    ...
    it 'reflects on canonical IDs' do
      ...
      expect(delivery).to receive(:reflect).with(:fcm_canonical_id, '2', 'canonical123')
      perform
    end
    ...
    it 'reflects on invalid IDs' do
      ...
      allow(response).to receive_messages(body: JSON.dump(body))
      allow(notification).to receive_messages(registration_ids: %w(1 2 3))
      expect(delivery).to receive(:reflect).with(:fcm_invalid_registration_id, app, 'NotRegistered', '2')
      perform_with_rescue
    end
