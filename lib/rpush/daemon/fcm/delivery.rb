      ...
      class Delivery < Rpush::Daemon::Delivery
        ...
        def handle_successes(successes)
          successes.each do |result|
            reflect(:fcm_delivered_to_recipient, @notification, result[:registration_id])
            next unless result.key?(:canonical_id)
            reflect(:fcm_canonical_id, result[:registration_id], result[:canonical_id])
          end
        end
        ...
        def handle_errors(failures)
          failures.each do |result|
            reflect(:fcm_failed_to_recipient, @notification, result[:error], result[:registration_id])
          end
          failures[:invalid].each do |result|
            reflect(:fcm_invalid_registration_id, @app, result[:error], result[:registration_id])
          end
        end
        ...
        def create_new_notification(response, unavailable_idxs)
          ...
          Rpush::Daemon.store.create_fcm_notification(attrs, @notification.data,
